   ...
   #%% 
    ...
    for cols in range(nT):
        for rows in range(ncondi): 
            ...
            #vmin = 1e6*np.min([np.min(time_avg_GOc),np.min(time_avg_GOu), np.min(time_avg_NoGo)])
            #vmax = 1e6*np.max([np.max(time_avg_GOc),np.max(time_avg_GOu), np.max(time_avg_NoGo)])
