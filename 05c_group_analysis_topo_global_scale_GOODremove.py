...
for i in range(nT): 
    time_avg_GOc = evoked_GOc.get_data(picks = 'eeg', tmin = tmin[i], tmax = tmax[i]).mean(axis = 1)
    time_avg_GOu = evoked_GOu.get_data(picks = 'eeg',tmin = tmin[i], tmax = tmax[i]).mean(axis = 1)
    time_avg_NoGo = evoked_NoGo.get_data(picks = 'eeg',tmin = tmin[i], tmax = tmax[i]).mean(axis = 1)
    vabsmax_all[:,i] = np.max([np.max(np.abs(time_avg_GOc)),np.max(np.abs(time_avg_GOu)), np.max(np.abs(time_avg_NoGo))])
...
for fiscl in scale: 
    ...
    for cols in range(nT):
        for rows in range(ncondi): 
            time_avg_GOc = evoked_GOc.get_data(picks = 'eeg', tmin = tmin[cols], tmax = tmax[cols]).mean(axis = 1)
            time_avg_GOu = evoked_GOu.get_data(picks = 'eeg',tmin = tmin[cols], tmax = tmax[cols]).mean(axis = 1)
            time_avg_NoGo = evoked_NoGo.get_data(picks = 'eeg',tmin = tmin[cols], tmax = tmax[cols]).mean(axis = 1)
            ## for column wise scale
            vabsmax = 1e6*np.max([np.max(np.abs(time_avg_GOc)),np.max(np.abs(time_avg_GOu)), np.max(np.abs(time_avg_NoGo))])
            vmaxTime = + vabsmax
            vminTime = - vabsmax
            if fiscl == 'timeScale':
                vmin = vminTime
            ...
            if rows == 0:
                im = evoked_GOc.plot_topomap(times = tavg[cols], average = tdiff[cols], vlim=(vmin, vmax),
                                              sphere = 0.45, axes = axs[rows, cols], colorbar=False)
                if cols == 0:
                    axs[rows, cols].set_ylabel("GOc")
            elif rows == 1:
                im = evoked_GOu.plot_topomap(times = tavg[cols], average = tdiff[cols],  vlim=(vmin, vmax),
                                              sphere = 0.45, axes = axs[rows, cols], colorbar=False,)
                if cols == 0:
                ...
                im = evoked_NoGo.plot_topomap(times = tavg[cols], average = tdiff[cols],  vlim=(vmin, vmax),
                                              sphere = 0.45, axes = axs[rows, cols], colorbar=False,)
                if cols == 0:
...
report.title = 'Group (n = ' + str(n_subs) + ') evoked topo_'+ evnt + ': ' + version 
extension = 'group_evoked_topo'
