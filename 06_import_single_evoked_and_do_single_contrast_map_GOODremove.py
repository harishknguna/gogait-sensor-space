...
"""
=============================================
Imports the MNE evokeds structure and do the contrast 
per subject
        ...
        for contra, contrast in enumerate(contrast_kind): # three contrasts
            ...
            for sub_num, subject in enumerate(config_for_gogait.subjects_list): 
                ...
                for ci, condi in enumerate(condi_name): 
                    ...
                    # store condi in dim1, chs in dim2, time in dim3  
                    evoked_array_per_sub_exp_dim = np.expand_dims(evoked_array_per_sub, axis = 0)                            
                    if ci == 0:
                        evoked_array_per_sub_all_condi = evoked_array_per_sub_exp_dim
                    else:
                        evoked_array_per_sub_all_condi = np.vstack((evoked_array_per_sub_all_condi, 
                                                                    evoked_array_per_sub_exp_dim))  
                    
                #%% # do the contrast per individual 
                # check the order condi_name = ['NoGo', 'GOc'] or ['NoGo', 'GOu'] or ['GOu', 'GOc']    
                evk_contrast_data  = evoked_array_per_sub_all_condi[0,:,:] - evoked_array_per_sub_all_condi[1,:,:]
                ...
                contrast_evoked.save(evoked_fname, overwrite=True)     
                
               
               
            # finally saving the report after the for subject loop ends.     
            print('Saving the reports to disk')  
            report.title = 'Single sub evoked contrast: ' + contrast + '_' + evnt+ '_' + version + '_' + ampliNormalization
            #report.title = 'Group sub STC contrast at ' + evnt
            extension = 'single_sub_evoked_contrast'
            report_fname = op.join(config_for_gogait.report_dir, config_for_gogait.base_fname_generic.format(**locals()))
            report.save(report_fname+'_' + contrast +'_'+ evnt +'_' + ampliNormalization + '_' + version+ '.html', overwrite=True)            
